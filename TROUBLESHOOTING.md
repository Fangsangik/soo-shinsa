# SooShinsa - 문제 해결 및 트러블슈팅 📝

이 문서는 SooShinsa 프로젝트 개발 과정에서 발생한 주요 문제들과 해결 방법을 정리한 문서입니다.

---

## 🔧 **최신 코드 품질 개선 및 성능 최적화 (2025.07)**

### 🚨 **1. 보안 취약점 해결 (Critical)**

#### **문제점**
- **민감정보 하드코딩**: JWT Secret, AWS 키, 결제 API 키가 `application.properties`에 노출
- **CORS 전체 허용**: `setAllowedOriginPatterns(List.of("*"))` 보안 위험
- **JWT 로그아웃 버그**: 전체 사용자 토큰 삭제로 인한 서비스 장애
...
products.forEach(p -> p.getBrand().getName()); // 각각 쿼리 실행
...
### 🔒 **3. 분산락 + DB락 데드락 문제 해결 (Critical)**

#### **문제점**
- **이중 락 사용**: Redisson 분산락 + PESSIMISTIC_WRITE DB락 동시 사용
- **데드락 발생**: 서로 다른 락 순서로 인한 교착상태
- **성능 저하**: SERIALIZABLE 격리 수준으로 과도한 대기
...
public OrdersResponseDto createOrder(...) {
    ProductOption option = repository.findByIdWithLock(id); // DB락
    // ...
}
...
public OrdersResponseDto createOrder(...) {
    // 원자적 업데이트로 경쟁 조건 방지
    int updatedRows = repository.decreaseStock(productOptionId, quantity);
    if (updatedRows == 0) {
...
- **동시 사용자**: 100명
- **테스트 지속시간**: 60초
- **테스트 시나리오**: 동시 주문, 쿠폰 적용, 재고 감소

#### **성능 개선 결과**
```
기존 성능: TPS 273 → 546 (오류율 50% → 1.43%)
개선 목표: TPS 800+ (오류율 0.5% 미만)
```

### 🛡️ **5. 보안 강화 요소**
- **.env.example**: 환경변수 템플릿 제공
- **CORS 헤더 제한**: 필요한 헤더만 허용
- **preflight 캐싱**: 성능 향상을 위한 3600초 캐시
...
**문제점**: 기존 구조에서는 브랜드 내에 카테고리가 포함되어 논리적으로 맞지 않음

**해결 방법**: 
- Category에서 SubCategory를 연결하고, SubCategory에서 Brand가 연결되도록 수정
- 카테고리와 서브 카테고리를 정규화하여 중복 데이터 감소, 참조 무결성 유지, 검색 및 조회 기능 성능 향상
...
### **🔍 문제점**
- 여러 개의 스레드(사용자)가 동시에 같은 ProductOption(상품 옵션)에 대해 주문을 진행할 때, 재고가 정상적으로 감소하지 않음
